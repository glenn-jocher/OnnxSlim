...
from onnxslim.onnx_graphsurgeon.ir.graph import Constant, Graph, Node
...
class PatternMapping(dict):
    ...
    def __init__(self, onnx_node=None) -> None:
        """Initializes a PatternMapping instance with optional ONNX node inputs and outputs."""
        super().__init__()
...
class GraphPattern:
    ...
    def __init__(self) -> None:
        """Initializes internal structures for managing graph patterns in ONNX models."""
        self.op = None  # op (str)
        ...
        """Assigns a unique tensor ID, tracks its input node if provided, and increments the total tensor count."""
    ...
    def _single_node_match(self, onnx_node: Node) -> bool:
        """Match the ONNX node with the pattern node based on op type and optional check function."""
