...
def format_bytes(size: Union[int, Tuple[int, ...]]) -> str:
    """Format byte sizes into human-readable strings with appropriate units (B, KB, MB, GB)."""
...
def is_converged(model: onnx.ModelProto, graph_ckpt, iter: int) -> bool:
    """Checks if the model's graph has converged to a stable state across iterations."""
